...
| Scoring fields on `FormFieldDescriptor` + `FormSubmission` | Phase 7 | Small |
...
| Structured admin builder UI (add/configure/reorder fields, live preview) | Phase 7 | Medium |
| RJSF + shadcn/ui renderer with custom widgets for workers | Phase 7 | Medium-High |
| AI features that consume `IFormSemanticContextBuilder` | Phase 17 | Follows naturally |
| WYSIWYG canvas builder upgrade (Joyfill-level drag-drop) | Phase N | When tenant feedback justifies |
...
Scoring is a first-class feature of the form engine. Standard in site audits, risk assessments,
contractor inspections, and compliance checklists.

### FormFieldDescriptor — scoring extensions

```csharp
public int? MaxScore { get; set; }
// boolean:  MaxScore = 10 → full marks if true, 0 if false
// enum:     score per option via ScoreMap
// number:   MaxScore caps the contribution

public Dictionary<string, int>? ScoreMap { get; set; }
// enum field: { "Good": 10, "Acceptable": 6, "Poor": 2, "Fail": 0 }
```

### FormTemplate — score thresholds

```csharp
public List<ScoreThreshold>? ScoreThresholds { get; set; }
// [ { Min: 90, Label: "Excellent",      Colour: "green" },
//   { Min: 70, Label: "Good",           Colour: "blue"  },
//   { Min: 50, Label: "Needs Attention",Colour: "amber" },
//   { Min: 0,  Label: "Critical",       Colour: "red"   } ]
```

### FormSubmission — computed on save

```csharp
public int? TotalScore { get; set; }
public int? MaxPossibleScore { get; set; }
public decimal? ScorePercentage { get; set; }   // 0.0 – 100.0
public string? ScoreRating { get; set; }        // resolved from ScoreThresholds
```

### Why scoring matters for AI

Instead of the AI reading 20 individual field values it sees:

```
Site B — Contractor Safety Inspection — 2026-05-17
Score: 62/100 (62%) — ⚠ NEEDS ATTENTION
Previous score (2026-04-15): 78% ↓ Declining trend

Non-compliant fields (IsComplianceCritical = true, scored 0):
• hasValidInduction — FALSE
• lastInductionDate — expired 13 months ago
```

Score + trend + non-compliant fields = one paragraph of AI-readable context that replaces
reading the entire submission.

---

## Builder UI — Phase 7 vs. Future

### Phase 7 — Structured Builder

A clean, functional admin page. Designed for trained super-admins, not general users.

```
┌───────────────────────────────────────────────────────┐
│  Form Template: Contractor Safety Induction           │
│  Industry: Construction  |  Standard: CDM 2015        │
│  ─────────────────────────────────────────────────── │
│  Fields                                [+ Add Field]  │
│  ⠿  Has Valid Induction?   boolean  ⚠  [Edit] [Del]  │
│  ⠿  Last Induction Date    date        [Edit] [Del]  │
│  ⠿  CA Signature           string      [Edit] [Del]  │
│                                                       │
│  ┌─────── Live Preview (RJSF renders) ─────────────┐ │
│  │  Has Valid Induction?   ○ Yes  ○ No             │ │
│  │  Last Induction Date    [01/01/2026         ]   │ │
│  │  CA Signature           [                   ]   │ │
│  └────────────────────────────────────────────────┘ │
...
Add field → configure in a side panel (display label, type, AiDescription, scoring,
compliance flag, EntityPropertyPath) → live preview updates instantly via RJSF.

Drag handle (⠿) for reorder. No WYSIWYG canvas. No multi-page tabs. Functional MVP.

### Phase 7 — Worker Renderer

RJSF + `@rjsf/shadcn-ui` theme + custom widgets. This is where the UI investment goes —
workers use this daily. Custom widgets replace RJSF defaults:

| Field type | Default (skip) | Custom widget |
|---|---|---|
| boolean | checkbox | Styled toggle switch |
| enum/select | `<select>` | Colour-coded pill selector |
| severity | `<select>` | Red/amber/green visual picker |
| date | text input | Calendar date picker |
| photo | — | Camera capture with preview |
| signature | — | Canvas touch signature pad |
| number | text input | Stepper with unit label |

Mobile-first. 48px minimum touch targets. Works with gloves.

### Future — WYSIWYG Canvas Builder

When tenant feedback shows the structured builder is limiting, upgrade to a full
drag-drop canvas (Joyfill quality). The renderer, schema format, and all semantic
metadata are identical — only the builder UI changes. The upgrade is cosmetic to the
data model, not architectural.

Architecture is designed so this upgrade path is open. Schema stored as JSON — same
format regardless of how it was built.
